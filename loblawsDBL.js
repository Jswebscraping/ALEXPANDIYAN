...
try{
    async function Mongo(arr1) {
        ...
        mydb.collection('LobLawsDB').insertMany(arr1[i],function (err,res) {
            if(err) console.log("Error",err);
    ...
    async function LobLaws() {
        ...
        const keyword = fs.readFileSync("./Keywords.csv",'utf-8');
        ...
        for(let i=0;i<keywords.length;i++) {
            ...
            const productName =  await page.$x(xpaths[i]);
            const productPrice = await page.$x(xpaths[i]);
            const productCompPrice = await page.$x(xpaths[i]);
